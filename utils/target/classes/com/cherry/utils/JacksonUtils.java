...
public enum JacksonUtils {
	 ...
	 * @return
	 */
	public <T> T readFromFile(File file, TypeReference<T> typeReference)throws IOException {
		if (file == null || !file.exists() || file.isDirectory() )
			throw new IllegalArgumentException("传入的file参数不合法");
		return  objectMapper.readValue(file, typeReference);
	}
	
	public <T> T readFromInputStream(InputStream input,TypeReference<T> typeReference)throws IOException{
		if (input == null)
			throw new IllegalArgumentException("传入的input参数不合法，为null");
		return objectMapper.readValue(input, typeReference);
	}
	
	public <T> T readFromInputStream(InputStream input,Class<T> valueType)throws IOException{
		if (input == null)
			throw new IllegalArgumentException("传入的input参数不合法，为null");
		return objectMapper.readValue(input, valueType);
	}
	
	public <T> T readFromInputStream(InputStream input,JavaType valueType)throws IOException{
		if (input == null)
			throw new IllegalArgumentException("传入的input参数不合法，为null");
		return objectMapper.readValue(input, valueType);
	}
