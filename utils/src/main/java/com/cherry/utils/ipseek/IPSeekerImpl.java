package com.cherry.utils.ipseek;
...
public class IPSeekerImpl implements IPSeeker{
	// 用来做为cache，查询一个ip时首先查看cache，以减少不必要的重复查找
		...
		private byte[] b4;
		private byte[] b3;
		private String dirPath = CommonMethod.getAbsoluteFilePath(this.getClassPath()+"com/cherry/resource");
		...
		public IPSeekerImpl(String dirPath, String filename) {
			super();
		...
		public String getCountry(byte[] ip) {
			...
			// 保存ip，转换ip字节数组为字符串形式
			String ipStr = IPSeekUtil.getIpStringFromBytes(ip);
		...
		public String getCountry(String ip) {
			return getCountry(IPSeekUtil.getIpByteArrayFromString(ip));
		}
		...
		public String getArea(String ip) {
			return getArea(IPSeekUtil.getIpByteArrayFromString(ip));
		}
		...
		private String readString(long offset) {
			try {
				...
				// ipFile.readByte());
				if (i != 0){
					return IPSeekUtil.getString(buf, 0, i, "GBK");
				}
			} catch (IOException e) {
				// LogFactory.log("", Level.ERROR, e);
			}
